        ...
        .then(res => {
          ...
          for (let i=0; i<newList.length; i++) {
            ...
            for (let j=0; j<newList[i].issued_assets.length; j++){
              if ((newList[i].issued_assets[j]) && (new Date(newList[i].issued_assets[j].investment_date) < new Date(newDate))) {
    ...
    return (
      ...
      {this.state.lists.map( list=>{
        return <div>
            <div className="lists" onClick={this.attachEvent}>
              ...
              <div>{`$ ${list.cost.$.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, "$&,")}`}</div>
            </div>

            <div className="content">
              {list.issued_assets.map(asset => {
                return <div className="sublists">
                    <div />
                    <div>{asset.asset_class}</div>
                    <div>{asset.investment_date}</div>
                    <div>
                      {accounting.formatMoney(asset.quantity)}
                    </div>
                    <div>{`$ ${asset.cost.$.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, "$&,")}`}</div>
                  </div>;
              })}
            </div>
          </div>;
      ...
      <form>
        <div>
          ...
          <input onChange={this.handleSelect} type="date" id="start"
